...
def main():
    ...
    with h5py.File(out/'display_data.h5','r') as h:
        ...
        model=MainFigure(h,meta);model.update(i);model.fig.canvas.draw()
        locations=model.flow.ax.transData.transform(np.array([[-1,tail[1]],tail,tip]))
        plt.close(model.fig)
    dest=out/'geometry_audit';dest.mkdir(exist_ok=True)
    raw=dest/(a.stage+'_decoded.png')
    subprocess.run(['ffmpeg','-v','error','-i',str(out/'bulk_flow_explained.mp4'),'-vf',f'select=eq(n\\,{i})',
