"""STEP3A8 bounded, source-frozen full-CHNS bridge and continuous L0 probe."""
import gc
import hashlib
import os
...
from ..full_rate_policy import POLICY,CONTROLS,L0_SHA,cost_forecast,session_result
...
ROOT=Path("validation_results/step3a8")
ISO=Path("validation_results/step3a7")
SOURCE=Path(__file__).resolve().parents[1]
RUNNERS=[Path("scripts/step3a8_benchmarks.py"),Path("scripts/step3a8_report.py"),Path("docker/step3/run.sh")]
...
def freeze():
    if (ROOT/"policy/frozen.json").exists(): return source_guard()
    ...
    bridge=read_json(ROOT/"algebraic_bridge/tiny_fem/status.json")
    if not bridge["passed"] or bridge["implementation"]!=implementation(): raise ValueError("Preflight source mismatch")
    ...
    frozen={"implementation":implementation(),"stack":stack(),"policy":POLICY,"policy_sha256":json_hash(POLICY),
        "design_sha256":file_hash("STEP3A8_DESIGN.md"),"inherited_policy":inherited,
        ...
        "isolated_reference_markers":refs["COMPLETE"],"tiny_bridge_sha256":file_hash(ROOT/"algebraic_bridge/tiny_fem/status.json")}
    ...
    with tarfile.open(ROOT/"policy/source.tar.gz","x:gz") as tar:
        ...
        tar.add("STEP3A8_DESIGN.md",arcname="STEP3A8_DESIGN.md")
...
def used(kind="total"):
    ...
    return sum(r["wall_s"] for r in values if kind=="total" or r.get(kind,False))
...
def one_step(name,formulation,dt,level):
    from ..full_rate_experiment import OneStepAudit,solve_one
    ...
    try:
        ...
        audit=OneStepAudit(e,f["inherited_policy"]["policy"],POLICY,f["target_mass"])
        ...
        result=solve_one(e,dt,CONTROLS[level],audit,"step3a8_"+name.replace("/","_")+"_")
...
def moderate():
    ...
    for level in POLICY["moderate_levels"]:
        ...
        atomic_json(ROOT/"moderate/comparison.json",{"comparisons":comparisons,
            "passed":len(comparisons)==len(POLICY["moderate_levels"]) and c["passed"],"temporal_qualification":False})
        print("moderate",level,c,flush=True); gc.collect()
    if not comparisons[-1]["passed"]: raise ScientificFailure("Moderate full formulation bridge unqualified")
    return comparisons
...
def restart_check():
    ...
    execute(28,folder=dst,resume=True,full_path=False)
