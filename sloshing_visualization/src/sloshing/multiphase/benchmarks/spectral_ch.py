...
def planner_stage():
    ...
    from ..ch_validation_policy import AnalysisBudget, run_sparse_analysis, linear_gates
    ...
    # STEP3A4 fixes the dependency even for this legacy entry point. Its old
    # full-CHNS series gate must never suppress independently affordable linear
    # verification. Historical outputs are not regenerated by this change.
    ...
    verification = run_sparse_analysis(lambda: verify_full_sparse_schedule(
        op, reduced, V, plan["blocks"], monitor, check_budget=analysis_budget.check),
        reduced_pass=True, analysis_budget=analysis_budget)
    write_json(output/"full_sparse_verification.json", verification)
    sparse_ok = all(linear_gates(verification, full_sparse=True).values())
    ...
    plan["reduced_linear_verified"] = True
    plan["full_sparse_linear_verified"] = sparse_ok
    plan["full_CHNS_series_execution_authorized"] = bool(cost_ok and sparse_ok)
    # Backward-compatible STEP3A3 field means ONLY its full-CHNS series, never
    # isolated CH/probe authority. STEP3A4 does not consume this legacy flag.
    plan["nonlinear_execution_authorized"] = plan["full_CHNS_series_execution_authorized"]
