...
def run_reference(run_dir, config, identity, heartbeat, stop_after=None):
    ...
    with h5py.File(path, "r+" if resumed else "x") as h:
        if resumed:
            ...
            expected_markers = Coating.initial(identity["run_id"], -config.d, *initial.eta[[0, -1]]).markers
            if (coating.time != state.time or coating.bottom != -config.d or
                    not np.array_equal(coating.H, last["H"][:]) or
                    not np.array_equal(last["wet_intervals"][:], [[-config.d, v] for v in coating.H]) or
                    coating.markers != expected_markers):
                raise ValueError("Checkpoint coating/time/marker identity invalid for pinned reference")
            expected_H = initial.eta[[0, -1]].copy()
            for key in keys:
                gr = groups[key]
                expected_H = np.maximum(expected_H, gr["eta"][:][[0, -1]])
                memory = json.loads(gr.attrs["coating"])
                if (not np.array_equal(gr["H"][:], expected_H) or memory["H"] != expected_H.tolist() or
                        memory["markers"] != [asdict(m) for m in expected_markers] or
                        gr.attrs["state_id"] != f"{identity['run_id']}:{key}"):
                    raise ValueError("Checkpoint history contains inconsistent retained state")
        else:
