...
def _review_signoff(output, run_id, native_digest, required_hashes):
    ...
    try:
        ...
        if review.get("native_audit") and "test_signed_native_audit_reuse_is_narrow" not in names:
            errors.append("missing_signed_native_handoff_negative_test_evidence")
...
def _signed_native_audit(output, manifest):
    """Narrow immutable evidence handoff, never a cache of solver diagnostics.

    Only an independently signed, completed full operator audit can be reused.
    Every native byte and identity is checked again before its report is used.
    The ordinary verify entry point never calls this unless explicitly enabled.
    """
    output = Path(output)
    review_path = output / "independent_review.json"
    if not review_path.is_file():
        raise ValueError("unsigned_native_audit")
    review = _json(review_path)
    binding = review.get("native_audit", {})
    if (review.get("status") != "PASS_DECLARED_EXTENSION" or review.get("visual_native_reviewed") is not True
            or review.get("run_id") != manifest["run_id"]
            or binding.get("path") != "native_audit.json"
            or binding.get("scope") != "FULL_STAGE_OPERATOR_NATIVE_AUDIT"):
        raise ValueError("unsigned_or_wrong_scope_native_audit")
    path = output / "native_audit.json"
    if not path.is_file() or sha256(path) != binding.get("sha256"):
        raise ValueError("signed_native_audit_hash_mismatch")
    audit = _json(path)
    if (audit.get("scope") != "DECLARED_EXTENSION_FINAL_ACCEPTANCE"
            or audit.get("scientific_checks_passed") is not True
            or audit.get("native_verification_mode", "FULL_OPERATOR_RECOMPUTATION") != "FULL_OPERATOR_RECOMPUTATION"):
        raise ValueError("native_audit_not_completed_full_operator_check")
    source_path = Path(audit["auditor_source_snapshot_path"])
    if not source_path.is_absolute():
        source_path = output / source_path
    source_digest = audit["auditor_source_sha256"]
    if source_digest != binding.get("auditor_source_sha256") or sha256(source_path) != source_digest:
        raise ValueError("native_auditor_source_snapshot_mismatch")
    saved = audit["native_reports"]
    if set(saved) != {item["run_id"] for item in manifest["native_runs"]}:
        raise ValueError("native_audit_run_set_mismatch")
    for item in manifest["native_runs"]:
        native = Path(item["path"])
        checked = saved[item["run_id"]]
        digest = sha256(native)
        if digest != item["sha256"] or digest != checked.get("native_sha256"):
            raise ValueError("signed_native_data_hash_mismatch:" + item["run_id"])
        with h5py.File(native, "r") as h:
            identity = json.loads(h.attrs["identity"])
            c = json.loads(h.attrs["config"])
            complete = bool(h.attrs.get("completed", False))
        if (checked.get("passed") is not True or checked.get("scope") != "DECLARED_NAVIER_NATIVE"
                or checked.get("failures") != [] or not complete
                or Path(checked["native"]).resolve() != native.resolve()
                or checked.get("run_id") != identity.get("run_id") or checked["run_id"] != item["run_id"]
                or checked.get("source_hash") != identity.get("source_hash")
                or checked.get("physical_config_hash") != identity.get("physical_config_hash")
                or checked.get("physical") != identity.get("physical") or checked["physical"] != _physical(c)
                or identity.get("source_hash") != _hash(identity["source_files"])
                or identity.get("physical_config_hash") != _hash(_physical(c))
                or identity.get("source_kind") != "declared_Navier_linear_FEM"
                or identity.get("synthetic") is not False or identity.get("physical_model_label") != LABEL):
            raise ValueError("signed_native_report_identity_or_scope:" + item["run_id"])
        if _json(native.parent / "identity.json") != identity:
            raise ValueError("signed_native_identity_file_mismatch")
        resolved = _json(native.parent / "resolved_case.json")
        if (resolved.get("controls") != c or resolved.get("physical") != _physical(c)
                or resolved.get("physical_model_label") != LABEL or resolved.get("film_thickness_m") is not None
                or resolved.get("retained_layer_volume_feedback") != "neglected_at_leading_order"
                or resolved["extension_contract"]["record_height_tolerance_m"] != .0001):
            raise ValueError("signed_native_resolved_case_mismatch")
        for name, source_sha in identity["source_files"].items():
            relative = Path(name)
            if relative.is_absolute() or ".." in relative.parts or sha256(native.parent / "source_snapshot" / relative) != source_sha:
                raise ValueError("signed_native_source_snapshot_mismatch")
    primary = Path(manifest["native_data_paths"][0])
    if (manifest["run_id"] not in saved or Path(saved[manifest["run_id"]]["native"]).resolve() != primary.resolve()
            or review.get("native_sha256") != saved[manifest["run_id"]]["native_sha256"]):
        raise ValueError("signed_native_primary_mismatch")
    return saved, {"path": str(path), "sha256": binding["sha256"],
                   "auditor_source_snapshot_path": str(source_path), "auditor_source_sha256": source_digest,
                   "scope": "FULL_STAGE_OPERATOR_NATIVE_AUDIT", "all_native_hashes_rechecked": True}
...
def _report_sections(report, output):
    ...
    report["physics"] = {"status": physics_status,
                         "evidence_paths": native_paths + paths(["boundary_benchmark.json", "native_audit.json"]),
                         "verification_mode": report.get("native_verification_mode", "FULL_OPERATOR_RECOMPUTATION")}
...
def verify_extension(output, *, use_signed_native_audit=False):
    """Verify a complete package; full operator recomputation is the default.

    Signed immutable native evidence may explicitly be reused for a renderer-only
    release. Export, refinement, memory, detector and video checks still rerun.
    """
    output = Path(output)
    errors, natives = [], {}
    def require(ok, reason):
    ...
    report = {"passed": False, "scientific_checks_passed": False,
              ...
              "native_verification_mode": "SIGNED_IMMUTABLE_NATIVE_AUDIT_REUSE" if use_signed_native_audit else "FULL_OPERATOR_RECOMPUTATION"}
    try:
        ...
        signed = None
        if use_signed_native_audit:
            signed, handoff = _signed_native_audit(output, m)
            report["native_audit_reuse"] = handoff
        for item in m["native_runs"]:
            path = Path(item["path"])
            require(sha256(path) == item["sha256"], "native_checksum:" + item["role"])
            checked = signed[item["run_id"]] if signed is not None else verify_native(path)
