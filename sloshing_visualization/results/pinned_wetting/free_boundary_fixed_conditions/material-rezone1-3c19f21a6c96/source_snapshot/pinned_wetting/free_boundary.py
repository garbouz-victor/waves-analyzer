...
class Controls:
    ...
    def __post_init__(self):
        numbers=(self.dt,self.t_end,self.minimum_dt,self.rezone_interval_s,self.nonlinear_tolerance)
...
class Operators:
    def __init__(self, mesh, c, advecting_velocity=None):
        ...
        self.f_body = asm(gravity, self.vbasis)
        self.f = (asm(surface_gravity,self.vbasis.boundary("surface",intorder=c.intorder))
                  if c.hydrostatic_split else self.f_body)
        self.Kg = csr_matrix(self.M.shape)
        if c.skew_divergence and advecting_velocity is not None:
            field = self.vbasis.interpolate(advecting_velocity)
            self.Kg = asm(divergence_correction,self.vbasis,
                          div_vm=field.grad[0,0]+field.grad[1,1]).tocsr()

    def solve(self, v0, dt):
        ids = self.free
        M, K, B = self.M[ids][:, ids], (self.Kb+self.Kl+self.Kg)[ids][:, ids], self.B[:, ids]
...
def step(mesh0, v0, dt, c, orientation):
    ...
    residual = op.M@((v1-v0)/dt)+(op.Kb+op.Kl+op.Kg)@vm-op.B.T@p-op.f
