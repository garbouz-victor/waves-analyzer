...
def resolved_left_record(previous,peak,current,retained_height,last_marker,threshold=1e-6):
    """Confirm a sampled local maximum only if it also sets the global record."""
    return previous<peak and current<peak and peak>last_marker+threshold and peak==retained_height
...
def identity(root, c):
    ...
    files = ["pinned_wetting/free_boundary.py", "pinned_wetting/free_boundary_run.py",
             "pinned_wetting/free_boundary_remesh.py", "pinned_wetting/free_boundary_mesh_quality.py",
             "pinned_wetting/coating.py", "mesh.py", "config.py"]
...
def run_case(root, c, role, state, heartbeat, stop_after=None):
    ...
    with h5py.File(native,"a") as h:
        if "states" not in h:
            ...
            save_csr(h.create_group("initial_grad_div_operator"),op0.Kd)
            ...
            row.update(cumulative_bulk_loss=0.,cumulative_left_wall_loss=0.,
                ...
                numerical_grad_div_work=0.,cumulative_numerical_grad_div_work=0.,
        ...
        reference_minima=fb.quadratic_minima(reference)[0]
        while t < c.t_end-1e-13 and (stop_after is None or n<stop_after):
            ...
            quality_due=False
            if c.rezone_strategy=="quality_optimized" and c.rezone_interval_s>0:
                quality_due=float(np.min(fb.quadratic_minima(mesh)[0]/reference_minima))<.01
            if c.rezone_interval_s>0 and (t>=last_rezone+c.rezone_interval_s-1e-11 or quality_due):
                ...
                last_rezone=t
                rezoning["trigger"]="min_relative_J_below_0.01" if quality_due else "scheduled_interval"
            ...
            for trial in range(c.max_halvings+1):
                try:
                    ...
                    for phase,geometry in (("midpoint",.5*(start_mesh.p+mesh1.p)),("endpoint",mesh1.p)):
                        if fb.surface_has_resolved_crossing(geometry[:,ids]):
                            raise ValueError(f"resolved full free-boundary self-intersection at {phase}")
                    break
                except (ValueError,RuntimeError) as error:
            ...
            if n>=2:
                ...
                # An unmarked plateau can already hold the global record.
                # A later lower local peak must not become a new marker.
                if resolved_left_record(oldR,lastR,newR,coating.H[0],last_marker,threshold):
                    coating.record_peak(f"P{len(coating.markers)+1}","L",float(lastR),t-dt,f"{run_id}:{n-1}")
            for suffix in ("bulk_loss","left_wall_loss","geometry_kinetic_work","geometry_potential_work","numerical_grad_div_work"):
            ...
            row["split_energy_error"] = row["continuous_energy_error"]-(row["cumulative_geometry_kinetic_work"]+row["cumulative_geometry_potential_work"]+row["cumulative_rezone_work"]+row["cumulative_numerical_grad_div_work"])/E0
            ...
            if n%10==0 or t>=c.t_end-1e-13:
                ...
                heartbeat(SimpleNamespace(step=n,time=t),str(native))
        result = {"run_id":run_id,"native":str(native),"time_s":float(t),"accepted_step":n,
            "completed":bool(t>=c.t_end-1e-13),"target_5s":bool(t>=5.-1e-13),"rejected_this_call":rejected,
...
def free_boundary_main(root, cfg, args):
    ...
    if args.command in ("run","pilot","resume"):
        ...
        c = fb.Controls(nx=12*2**args.mesh_level,nz=24*2**args.mesh_level,
            ...
            rezone_strategy=args.rezone_strategy,grad_div_gamma_m2_s=args.grad_div_gamma)
        if args.run_id:
