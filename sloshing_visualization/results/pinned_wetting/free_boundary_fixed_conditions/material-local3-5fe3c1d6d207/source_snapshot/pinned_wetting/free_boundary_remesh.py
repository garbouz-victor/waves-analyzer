...
def solve_transfer(mass, divergence, momenta, rhs, target_momentum):
    """Exact small Schur elimination of the two GLOBAL momentum constraints.

    Factor the sparse velocity/pressure saddle once; do not let two dense
    constraint rows cause global LU fill. All pressure DOFs and both momentum
    constraints remain in the model and in the independently checked witness.
    An inaccurate/degenerate Schur solve falls back to the original full solve.
    """
    nv, npres = mass.shape[0], divergence.shape[0]
    C = vstack((divergence, csr_matrix(momenta)), format="csr")
    full = bmat([[mass, C.T], [C, None]], format="csc")
    target = np.r_[rhs, np.zeros(npres), target_momentum]
    stats = {"method": "two_global_constraints_schur", "full_direct_fallback": False,
             "schur_condition_number": None, "schur_numerical_rank": None,
             "schur_solver": "dense_2x2"}
    solution = None
    try:
        local = bmat([[mass, divergence.T], [divergence, None]], format="csc")
        factor = splu(local, permc_spec="COLAMD")
        loads = np.zeros((nv+npres, 3))
        loads[:nv, 0] = rhs
        loads[:nv, 1:] = momenta.T
        answers = factor.solve(loads)
        ordinary, responses = answers[:, 0], answers[:, 1:]
        schur = momenta@responses[:nv]
        condition = float(np.linalg.cond(schur))
        stats["schur_condition_number"] = condition if np.isfinite(condition) else None
        schur_rhs = momenta@ordinary[:nv]-target_momentum
        rank = int(np.linalg.matrix_rank(schur))
        stats["schur_numerical_rank"] = rank
        if rank < 2:
            # A straight tilted boundary can make Pz-slope*Px dependent on
            # weak divergence. Solve a consistent dependent algebraic system;
            # NEVER drop its original constraint or add a physical penalty.
            multipliers = np.linalg.lstsq(schur, schur_rhs, rcond=None)[0]
            stats["schur_solver"] = "rank_revealing_least_squares"
        else:
            multipliers = np.linalg.solve(schur, schur_rhs)
        candidate = np.r_[ordinary-responses@multipliers, multipliers]
        if np.all(np.isfinite(candidate)) and np.max(abs(full@candidate-target)) <= 1e-12:
            solution = candidate
    except (np.linalg.LinAlgError, RuntimeError, ValueError):
        pass
    if solution is None:
        stats["full_direct_fallback"] = True
        solution = spsolve(full, target)
    error = float(np.max(abs(full@solution-target)))
    if not np.all(np.isfinite(solution)) or not np.isfinite(error) or error > 1e-9:
        raise ValueError("conservative transfer mixed solve failed")
    stats["full_mixed_residual"] = error
    return solution, stats


def rezone(reference,mesh,velocity,c,orientation):
    linear=c.rezone_strategy in ("straight_interior","quality_optimized")
    ...
    # Impose the prescribed mesh-Dirichlet boundary during construction,
    # including roundoff from reference+displacement in the harmonic option.
    new.p[:,surface]=mesh.p[:,surface]
    optimizer_statistics=None
    if c.rezone_strategy=="quality_optimized":
        from .free_boundary_mesh_quality import optimize_mesh
        # Dimensionless mesh-quality trigger; not a physical film width and
        # not related to the Navier length. Do not optimize nearly undeformed
        # cells needlessly. All tested candidates keep the full curve fixed.
        relative=float(np.min(quadratic_minima(new)[0]/quadratic_minima(reference)[0]))
        if relative<.2:
            new,optimizer_statistics=optimize_mesh(reference,new,orientation,max_iterations=80)
            candidate_name+="+quality_optimized"
        else:
            optimizer_statistics={"skipped":True,"reason":"min_relative_J_at_least_0.2",
                                  "selected_min_relative_jacobian":relative}
    new,v,evidence=transfer_mesh(mesh,new,velocity,c,orientation)
    evidence.update(strategy=c.rezone_strategy,candidate=candidate_name,fraction=fraction)
    if optimizer_statistics is not None:
        evidence["mesh_optimizer_statistics"]=json.dumps(optimizer_statistics)
    return new,v,evidence


def transfer_mesh(mesh,new,velocity,c,orientation):
    """Conservative same-topology/same-boundary transfer, independently auditable."""
    if not np.array_equal(mesh.t,new.t) or mesh.p.shape!=new.p.shape:
        raise ValueError("same-boundary rezone cannot silently change topology")
    ...
    if not np.array_equal(new.p[:,surface],mesh.p[:,surface]):
        raise ValueError("rezone moved the physical free boundary")
    for name,axis in (("left",0),("right",0),("bottom",1)):
        nodes=scalar.get_dofs(name).all()
        if not np.array_equal(new.p[axis,nodes],mesh.p[axis,nodes]):
            raise ValueError("rezone moved a solid wall in its normal direction")
    ...
    solution,linear_statistics=solve_transfer(op.M[free][:,free],op.B[:,free],P[:,free],rhs[free],target_momentum)
    ...
    evidence={"geometry":new.p.copy(),"velocity":v,"projection_multipliers":solution[len(free):],
        ...
        "linear_projection_statistics":json.dumps(linear_statistics)}
    return new,v,evidence
