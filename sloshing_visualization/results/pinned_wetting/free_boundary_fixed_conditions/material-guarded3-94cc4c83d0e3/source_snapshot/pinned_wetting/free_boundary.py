...
def adjacent_quadratic_loop(points):
    """Additional analytic adjacent-edge guard, independent of verifier roots.

    From the shared join write A(u)=a*u+b*u**2, B(v)=c*v+d*v**2.
    For u,v>0, v=r*u gives the cubic cross(a-c*r,b-d*r*r)=0.
    The known join is divided out algebraically, not deleted by a radius.
    Exact rational native coefficients identify degeneracies/shared tangencies;
    numerical cubic roots are checked in the original vector equations. Both
    orientations use 0<r<=1 to avoid unbounded parameter ratios. This is still
    an early numerical guard, not a substitute for the independent full audit.
    """
    p=[[Fraction(float(x)) for x in row] for row in np.asarray(points)]
    a=tuple(-3*p[k][2]+4*p[k][1]-p[k][0] for k in range(2))
    b=tuple(2*(p[k][0]+p[k][2]-2*p[k][1]) for k in range(2))
    c=tuple(-3*p[k][2]+4*p[k][3]-p[k][4] for k in range(2))
    d=tuple(2*(p[k][2]+p[k][4]-2*p[k][3]) for k in range(2))
    def cross(x,y):return x[0]*y[1]-x[1]*y[0]
    def turning(x,y):
        if not any(x) and not any(y):return True
        k=0 if y[0] else 1
        if not y[k] or cross(x,y):return False
        q=-x[k]/(2*y[k])
        return 0<=q<=1
    if turning(a,b) or turning(c,d):return True
    def extent(x,y,k):
        values=[Fraction(0),x[k]+y[k]]
        if y[k]:
            q=-x[k]/(2*y[k])
            if 0<q<1:values.append(x[k]*q+y[k]*q*q)
        return min(values),max(values)
    def oriented(a,b,c,d):
        coef=[cross(a,b),-cross(c,b),-cross(a,d),cross(c,d)]
        while coef and not coef[-1]:coef.pop()
        if not coef:
            directions=(a,b,c,d)
            axis=next((v for v in directions if any(v)),None)
            if axis is None:return True
            # Distinct straight traces through J meet only at their known join.
            if any(cross(axis,v) for v in directions):return False
            k=0 if axis[0] else 1
            one,two=extent(a,b,k),extent(c,d,k)
            low,high=max(one[0],two[0]),min(one[1],two[1])
            return low<high or (low==high and low!=0)
        if any(c) and not cross(a,c):
            k=0 if c[0] else 1
            r0=a[k]/c[k]
            if all(a[j]==r0*c[j] for j in range(2)):
                if all(b[j]==r0*r0*d[j] for j in range(2)) and r0>0:
                    return True
                # Only exact factors associated with u=v=0 are removed.
                while len(coef)>1:
                    quotient=[Fraction(0)]*(len(coef)-1)
                    quotient[-1]=coef[-1]
                    for j in range(len(quotient)-2,-1,-1):
                        quotient[j]=coef[j+1]+r0*quotient[j+1]
                    if coef[0]+r0*quotient[0]:break
                    coef=quotient
        if len(coef)<2:return False
        scale=max(abs(x) for x in coef)
        roots=np.polynomial.polynomial.polyroots([float(x/scale) for x in coef])
        af,bf,cf,df=(np.array([float(x) for x in v]) for v in (a,b,c,d))
        for root in roots:
            if abs(root.imag)>1e-8*(1+abs(root.real)):continue
            r=float(root.real)
            if not 0<r<=1+128*np.finfo(float).eps:continue
            aa=af-r*cf;bb=bf-r*r*df
            denominator=float(bb@bb)
            if not denominator:
                if not np.any(aa):return True
                continue
            u=-float(aa@bb)/denominator;v=r*u
            if not (0<u<=1+128*np.finfo(float).eps and 0<v<=1+128*np.finfo(float).eps):continue
            residual=af*u+bf*u*u-cf*v-df*v*v
            size=max(np.linalg.norm(af*u),np.linalg.norm(bf*u*u),
                     np.linalg.norm(cf*v),np.linalg.norm(df*v*v),np.finfo(float).tiny)
            if np.linalg.norm(residual)<=2048*np.finfo(float).eps*size:return True
        return False
    return oriented(a,b,c,d) or oriented(c,d,a,b)
...
def surface_has_resolved_crossing(points, subdivisions=64):
    ...
    Adjacent quadratic loops also have a separate analytic guard: the last
    chord/first chord can hide a small loop next to a shared vertex. General
    tangencies/nonadjacent sub-tessellation features are not certified here.
    """
    for edge in range(0,points.shape[1]-4,2):
        if adjacent_quadratic_loop(points[:,edge:edge+5]):return True
