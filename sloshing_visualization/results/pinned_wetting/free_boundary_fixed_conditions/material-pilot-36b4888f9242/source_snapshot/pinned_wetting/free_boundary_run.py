...
def save_state(h, step, t, mesh, v, p, coating, row):
...
def run_case(root, c, role, state, heartbeat, stop_after=None):
    ...
    with h5py.File(native,"a") as h:
        ...
        while t < c.t_end-1e-13 and (stop_after is None or n<stop_after):
            dt = min(dt_next,c.t_end-t)
            for trial in range(c.max_halvings+1):
                try:
                    mesh1,v1,p,row = fb.step(mesh,v,dt,c,orientation)
            ...
            row["split_energy_error"] = row["continuous_energy_error"]-(row["cumulative_geometry_kinetic_work"]+row["cumulative_geometry_potential_work"])/E0
            save_state(h,n,t,mesh1,v1,p,coating,row)
...
def free_boundary_main(root, cfg, args):
    ...
    if args.command in ("run","pilot","resume"):
        ...
        c = fb.Controls(nx=12*2**args.mesh_level,nz=24*2**args.mesh_level,
            ...
            local_right_levels=args.right_refine)
        if args.run_id:
